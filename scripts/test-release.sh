...
unzip -p "$fixture/dist/helheim-1.0.99.jar" helheim.theme.json | jq -e '.editorScheme == "Helheim"' >/dev/null
[[ $(unzip -p "$fixture/dist/helheim-1.0.99.jar" META-INF/plugin.xml | xmlstarlet sel -t -v '/idea-plugin/extensions/bundledColorScheme[@id="Helheim"]/@path') == /helheim.xml ]]
unzip -p "$fixture/dist/helheim-1.0.99.jar" helheim.theme.json | jq -e '.ui.ToolWindow["HeaderTab.padding"] == "4,10,4,10"' >/dev/null
...
for scenario in missing_registration mismatched_id mismatched_name; do
  ...
  case $scenario in
    ...
    mismatched_name) xmlstarlet ed -L -u '/scheme/@name' -v Other "$fixture/resources/helheim.xml" ;;
  esac
  ...
  cp "$project_root/resources/helheim.xml" "$fixture/resources/helheim.xml"
  printf 'Passed: %s\n' "$scenario"
done
